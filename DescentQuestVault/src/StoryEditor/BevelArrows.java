...
public class BevelArrows
{
    ...
    Arrow[] arrows = { new LineArrow(), new CurvedArrow(new Point(50,0),new Point(130,10)) };
    ...
    static class CurvedArrow  implements Arrow
    {
    	...
    	private Point size;
    	private float arrowLength = 80.0f;
    	
    	public CurvedArrow(Point positon, Point size) {
    		position=positon;
    		this.size=size;
    		arrowLength=(float) Math.sqrt(Math.pow(size.y,2)+Math.pow(size.x, 2));
    	}
        ...
        public void draw ( Graphics2D g )
        {
            ...
            //g.setColor ( Color.RED );
            g.draw ( new Line2D.Float ( position.x, position.y, position.x+size.x, position.y+size.y ) );
        }
    }
}
